...
** - Khóa "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options" chứa danh sách các tiến trình
...
** - Kỹ thuật này cũng đc dùng để lợi dụng các chương trình trợ năng (Accessibility Programs)
	=> VD: Set "Debugger" cho "sethc.exe" thành: "C:\Windows\System32\cmd.exe" hoặc "C:\Windows\System32\Taskmgr.exe"
	Do các phiên bản Windows mới kiểm tra chặt với sethc.exe, thay vào đó lợi dụng cmd.exe/Taskmgr.exe
** PoC:
