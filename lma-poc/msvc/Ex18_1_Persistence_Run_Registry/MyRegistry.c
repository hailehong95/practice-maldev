...
BOOL CreateRegistryKey(HKEY hKeyParent, PWCHAR subkey)
{
	...
	Ret = RegCreateKeyEx(hKeyParent, subkey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, &dwDisposition);
...
BOOL WriteDwordToRegistry(HKEY hKeyParent, PWCHAR subkey, PWCHAR valueName, DWORD data)
{
	...
	if (Ret == ERROR_SUCCESS) {
		...
		if (ERROR_SUCCESS != RegSetValueEx(hKey, valueName, 0, REG_DWORD, (LPBYTE)&data, sizeof(data))) {
			RegCloseKey(hKey);
			return FALSE;
		}
		// Close the key
